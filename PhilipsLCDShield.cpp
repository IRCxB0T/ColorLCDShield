/**
 *  PhilipsLCDShield.cpp - Arduino Library to control a Nokia 6100 LCD,
...
void PhilipsLCDShield::init()
{
	...
	LCDCommand(INVOFF);     // Turn off display inversion
...
void PhilipsLCDShield::clear(int color)
{
	LCDCommand(PASETP);
	LCDData(0);
	LCDData(131);
...
void PhilipsLCDShield::setChar(char c, int x, int y, int fColor, int bColor)
{
...
void PhilipsLCDShield::setPixel(int color, unsigned char x, unsigned char y)
{
	y	=	(COL_HEIGHT - 1) - y;
	x = (ROW_LENGTH - 1) - x;
...
void PhilipsLCDShield::contrast(char setting)
{
...
void PhilipsLCDShield::off(void)
{
	LCDCommand(DISPOFF);
